...
(window._gsQueue || (window._gsQueue = [])).push( function() {
	...
	window._gsDefine("TimelineMax", ["TimelineLite","TweenMax","easing.Ease"], function(TimelineLite, TweenMax, Ease) {
		...
		p.addCallback = function(callback, position, params, scope) {
			return this.add( TweenMax.delayedCall(0, callback, params, scope), position);
		};
		...
		p.tweenTo = function(position, vars) {
			...
			t = new TweenMax(this, (Math.abs(Number(copy.time) - this._time) / this._timeScale) || 0.001, copy);
 ...
 */
	...
	window._gsDefine("TimelineLite", ["core.Animation","core.SimpleTimeline","TweenMax"], function(Animation, SimpleTimeline, TweenMax) {
		...
		p.to = function(target, duration, vars, position) {
			return this.add( new TweenMax(target, duration, vars), position);
		};

		p.from = function(target, duration, vars, position) {
			return this.add( TweenMax.from(target, duration, vars), position);
		};

		p.fromTo = function(target, duration, fromVars, toVars, position) {
			return this.add( TweenMax.fromTo(target, duration, fromVars, toVars), position);
		};
		...
		p.staggerTo = function(targets, duration, vars, stagger, position, onCompleteAll, onCompleteAllParams, onCompleteAllScope) {
			...
			if (typeof(targets) === "string") {
				targets = TweenMax.selector(targets) || targets;
			}
			...
			for (i = 0; i < targets.length; i++) {
				...
				tl.add( new TweenMax(targets[i], duration, _copy(vars)), i * stagger);
			}
		...
		p.set = function(target, vars, position) {
			...
			return this.add( new TweenMax(target, 0, vars), position);
		};
		...
		TimelineLite.exportRoot = function(vars, ignoreDelayedCalls) {
			...
			while (tween) {
				next = tween._next;
				if (!ignoreDelayedCalls || !(tween instanceof TweenMax && tween.target === tween.vars.onComplete)) {
		...
		p.add = function(value, position, align, stagger) {
			...
			if (!(value instanceof Animation)) {
				if (value instanceof Array) {
					...
					value = TweenMax.delayedCall(0, value);
				} else {
		...
		p.getChildren = function(nested, tweens, timelines, ignoreBeforeTime) {
			...
			while (tween) {
				if (tween._startTime < ignoreBeforeTime) {
					//do nothing
				} else if (tween instanceof TweenMax) {
					if (tweens !== false) {
						a[cnt++] = tween;
					}
				} else {
		...
		p.getTweensOf = function(target, nested) {
			var tweens = TweenMax.getTweensOf(target),
				i = tweens.length,
				a = [],
				cnt = 0;
			while (--i > -1) {
